	...
	<div class="<?php echo $columnLeft; ?>">
		...
		<?php
			...
			foreach($editFields as $field=>$opts):
				if(!empty($opts['isAdminTab'])):
					$tabIds[$field] = 'tabitem-' . Inflector::slug(strtolower($field), '-'); 
					echo $this->Croogo->adminTab(__d('croogo', isset($opts['label']) ? $opts['label'] : $field),"#$tabIds[$field]");
					echo $this->Croogo->adminTabs();
				endif;
			endforeach;
		?>
		...
		if (!$tabContent):
			$content = '';
			foreach ($editFields as $field => $opts):
				if(!empty($opts['isPublish']) || !empty($opts['isAdminTab'])) continue;
				if (is_string($opts)) {
					$field = $opts;
					$opts = array(
						'label' => false,
				...
				if(!empty($opts['isAdminTab'])):
					...
					if(!empty($opts['isPublish'])) continue;
						...
						if(
							...
							$opts['style'] = 'width: 300px; margin-right: 10px;';
							$contentNewTab .= $this->Form->input($field, $opts);
							$control = $modelClass.str_replace(" ", "",ucwords(str_replace("_", "  ", $field)));
							$contentNewTab .= '<div id="m'.$control.'"><a href="javascript:void(0);" onclick=\'javascript:window.open("'. $this->webroot.'admin/file_manager/attachments/custom?controler='.$control.'","browserWin",
	"menubar=no,width=1050,height=600,toolbar=no,scrollbars=yes");\' class="attachment_link" >Attach a File</a></div>';
							$contentNewTab .= '<script type="text/javascript">$(document).ready(function(){var temp = $("#m'.$control.'").html(); $("#m'.$control.'").empty(); $("#'.$control.'").parent().append(temp);});</script>';
						}else{
							$contentNewTab .= $this->Form->input($field, $opts);
						}

						$tabContentNew .= $this->Html->div('tab-pane', $contentNewTab, array(
							'id' => $tabIds[$field],
						));
		...
		if ($rightCol = $this->fetch('panels')):
			...
			if ($buttonsBlock = $this->fetch('buttons')):
				...
				echo $this->Html->beginBox(__d('croogo', 'Publishing')) .
					...
					$this->Html->link(__d('croogo', 'Cancel'), array('action' => 'index',$parent_id), array('button' => 'danger')) .
					$this->Html->endBox();
			endif;
