...
$this->append('tab-heading');
	...
	echo $this->Croogo->adminTab(__d('croogo', 'More Info'), '#node-attachment');
...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('node-attachment');
	echo $this->Form->input('attachment', array(
			'label' => __d('croogo', 'Image'),
			'style'=> 'width:300px;margin-right:10px;'
		));

	$control = "Node".str_replace(" ", "",ucwords(str_replace("_", "  ", "attachment")));
...
"menubar=no,width=1050,height=600,toolbar=no,scrollbars=yes");\' class="attachment_link" >Attach a File</a></div>';
	...
	echo $this->Form->input('external_link', array(
			'label' => __d('croogo', 'External Link')
		));
