...
$this->start('table-heading');
	$tableHeaders = $this->Html->tableHeaders(array(
		...
		$this->Paginator->sort('attachment', __d('croogo', 'Attachment')),
...
$this->append('table-body');
	...
	foreach ($blocks as $block) {
		...
		if ($block['Block']['status'] == CroogoStatus::PREVIEW) {
		 ...
		 if(!empty($block['Block']['attachment'])):

				 $block_attachment = $this->Html->link($this->Image->resize('/uploads/'.$block['Block']['attachment'], 100, 75), $this->Html->url('/uploads/'.$block['Block']['attachment'], true), array(
										'class' => 'thickbox',
										'escape' => false,
										'title' => 'Image'
									 ));
			 	...
			 	$block_attachment = "no attachment";
